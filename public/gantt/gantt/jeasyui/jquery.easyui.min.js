...
var opts=$.data(_137,"tree").options;
    // add by grain 20181123
    //_138=$.extend({},opts.queryParams,_138||{});
    // 添加扩展查询参数
    _138 = $.extend({}, opts.extraParams || {}, opts.queryParams, _138 );
    // add end
var _13a=null;
if(_137!=ul){
var node=$(ul).prev();
...
opts.queryParams=_7ae;
    ...
    var _7af=$.extend({}, opts.extraParams || {}, opts.queryParams);
    // add end
if(opts.pagination){
...
if(col.formatter){
    ...
    if (typeof col.formatter === "string") {
        if (col.formatter.toLowerCase() === "ymd") {
            r = major.dateFormatYMD(_829);
        } else if (col.formatter.toLowerCase() === "ymdhms") {
            r = major.dateFormatYMDHms(_829);
        } else {
            console.log("unrecognized formatter: ", col.formatter);
        }
    } else {
        r = col.formatter(_829,_826,_825);
    }
    cc.push(r);
...
opts.queryParams=_92b;
    ...
    var _92e=$.extend({},opts.extraParams || {}, opts.queryParams);
    // add end
if(opts.pagination){
...
_982=_982 ? ($.isArray(_982) ? _982 : [_982]) : ["checked"];
var rows=[];
$.easyui.forEach(jq.data("treegrid").checkedRows,false,function(row){
if(_982.indexOf(row.checkState)!==-1){
rows.push(row);
}
});
return rows;
...
if(col.formatter){
    if (typeof col.formatter === "string") {
        if (col.formatter.toLowerCase() === "ymd") {
            val = major.dateFormatYMD(row[_99f]);
        } else if (col.formatter.toLowerCase() === "ymdhms") {
            val = major.dateFormatYMDHms(row[_99f]);
        } else {
            console.log("Unrecognized formatter: ", col.formatter);
        }
    } else {
