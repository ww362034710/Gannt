        ...
        prepareOption: function(opts) {
            ...
            switch (configOption.xAxis) {
                case 'productCode':
                    ...
                    // 结论数据
                    var preparedData = this.prepareData();    // [[14,20,18..], [29,33,..]..]
                    var series = [];
                    // 检测通道数量与设置的最大Y轴数量
                    ...
                    option = {
                        ...
                        tooltip: {
                            ...
                            formatter: function (params) {
                                var param = params[0];
                                console.log(dataValueType, productCodeNameList, params);
                                var ret ="";
                                if(dataValueType=='string'){
                                    ret = productCodeNameList[0][param.value[0]];
                                    $.each(params,function (i,item) {
                                        ret = ret + "<br>" +item.seriesName + ": " + item.value
                                    })
                                }else{
                                    ret = productCodeNameList[0][param.value[0]];
                                    $.each(params,function (i,item) {
                                ...
                                //var ret =  dataValueType=='string'? productCodeNameList[0][param.value[0]] + "(" + params.value + ")": productCodeNameList[0][param.value[0]] + "(" + params.value + ")";
                                return ret;
                            },
                            axisPointer: {
                                animation: false
                ...
                case 'testProject':
                    ...
                    if(!selectedBatchTasks || selectedBatchTasks.length == 0) {
                        return {
                            success : false,
                            msg : '请选择产品编号'
                        };
                            ...
                            $.each( xNameIdList , function( testProjectIdIndex , testProjectId ) {
                                ...
                                if (taskId) {
                                    ...
                                    if (taskData) {
                                        ...
                                        if (data) {
                                            seriesData.push(data.measuredValue);
                                        } else {
                                            seriesData.push('-');
                                        }
                                    }
