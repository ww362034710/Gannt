!function() {
    var Manager = function(opts) {
        ...
        this.selectedBatchTasks = null;   // 选中的批次任务+产品编号   [{id: 1, name: '生一批', productCodes: [{id:索引, name:x, nodeType:'productCodde', parentId:xx, taskId: taskId}...]},..]
        ...
        this.legends = null;
        this.legendsTheoValue = null;
    };

    $.extend(true, Manager.prototype, {
        refresh: function() {
            this.selectedBatchTasks = this.getSelectedBatchTasks();
            this.selectedTestProjects = this.getYAxisSelectedChannels();
            this.multiTestProject = this.selectedTestProjects.length>1;
            this.selectedXAxisTestProjects = configBucket.xAxis == "testProject" ? xAxisSelector.data.getSelectedTestProjects() : [];
        ...
        getSelectedBatchTasks: function() {
            return productTreeManager.data.getSelectedBatchTasks.call(productTreeManager);
        },
        getYAxisSelectedChannels: function() {
            return yAxisSelector.data.getSelectedChannels.call(yAxisSelector);
        },
        ...
        prepareTaskData: function() {
            ...
            $.each(this.selectedBatchTasks, function(_i, batchTask){
                var productCodes = batchTask.children;
                $.each(productCodes, function(i, productCodeRow) {
                    var taskIdMap = productCodeRow.taskId;     // {试验项目id: taskId...}
                    if (taskIdMap ) {
                        $.each(taskIdMap, function(testProjectId, taskId) {
                            if (self.CATCH_DATA.hasOwnProperty(taskId)) {
                                self.datas[taskId] = self.CATCH_DATA[taskId];
                            } else {
