(function(window, $) {
    var MCombotreegrid = new Class({
        ...
        extendOptions: function () {
            ...
            if (displayFieldsList && displayFieldsList.length > 0) {
                ...
                displayFieldsList[0] = [{name: 'ck', checkbox: true, rowspan: displayFieldsList.length}, {
                    name: 'id',
                    title: 'id',
                    type: 'long',
                    hidden: true,
                    rowspan: displayFieldsList.length
                }].concat(displayFieldsList[0]);
                        ...
                        var columnObj = $.extend({width: 100, align:'center'},
                            {
                                ...
                                width: column.width,
                                title: column.title,
                                align: column.align || (column.name === domain.displayField ? "left" : undefined),
    ...
    $.fn.mcombotreegrid = function(opts, args) {
        if (typeof opts === "string") {
            return this.data("m")[opts](this, args);
        } else if (!this.data("m")) {
            this.data("m", new MCombotreegrid($(this), opts));
        }
    };
})(window, jQuery);