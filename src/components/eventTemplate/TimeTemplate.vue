            ...
            <div>
                <a-form-model :model="form" layout="inline" :label-col="{span:6}" :wrapper-col="{span:18}">
                    <a-form-model-item label="约束类型" class="logic-form-1">
                        <a-select v-model="form.timing" placeholder="请选择约束条件" :options="this.TimingData">
                        </a-select>
                    </a-form-model-item>

                    <a-form-model-item label="示意图" class="logic-form-2" v-show="timingImg">
                        <div class="logic-view">
                            <img :src="timingImg" alt="" @click="showImgView()">
                        </div>
                    </a-form-model-item>
                </a-form-model>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            selectedKeys: [],
            tblRules: [],
            selTblData: [], // 该事件模板已经选择的时序约束(所有  不止于"已选择"表格中当前页的数据)
        }
    },
    watch: {
        'form.timing': {
            handler(value) {
                ...
                try {
                    //根据约束名字查找对象图片
                    img = require('@/assets/images/timing/' + value + '.png')
                } catch (e) {
                    console.error('无约束图片')
                }
                this.timingImg = img
