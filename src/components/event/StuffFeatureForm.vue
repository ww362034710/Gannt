...
<template>
    <div>
        ...
        <div v-auto:[autoLayout] data-diff="210">
            <a-table :columns="columns" :pagination="pagination" :loading="loading" :dataSource="selTblData" bordered size="middle" :scroll="{y:true,x:500}" rowKey="uuid">
                ...
                <a-form-item style="margin-bottom:0" slot="value" :validate-status="getFildStatus(record.uuid,'value').validateStatus" :help="getFildStatus(record.uuid,'value').errorMsg" slot-scope="text, record">
                    <div class="gathering-tbl">
                        <a-input-number :min="0" allowClear v-model="record.value" @change="handleChange(record.value, record.uuid,'value')" />
        ...
        <a-modal v-model="selRequire" title="资源功能列表" :width="800" ok-text="确认" cancel-text="取消" @ok="setSelRows()">
            <stuff-feature-selection :selected-keys="selectedKeys" ref="stuffFeatureSelection" />
        </a-modal>
    </div>
</template>
<script>
