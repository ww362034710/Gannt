<template>
  <div class="page-main">
    <div class="page-main-container">
      ...
      <!-- 新增model -->
      <a-modal
        :title="!showDetail?'新增用户':'用户详情'"
        v-model="addModelVisible"
        :maskClosable="false"
        :confirmLoading="confirmLoading"
        @ok="handleSubmit"
        :okText="!showDetail?'保存':'关闭'"
        cancelText="取消"
      >
        <a-form :form="addForm">
          <a-form-item label="用户昵称：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            <a-input
              :disabled="showDetail"
              v-decorator="['userName', addFormRule.userName]"
              placeholder="请填写昵称"
            />
          </a-form-item>
          ...
          <a-form-item label="归属部门：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            <a-tree-select
              ...
              :treeData="deptTreeArray"
              placeholder="请选择部门"
              treeDefaultExpandAll
              v-decorator="['deptId', addFormRule.deptId]"
            >
            <span
              style="color: #08c"
              slot="title"
          ...
          <a-form-item label="手机号码：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            <a-input
              :disabled="showDetail"
              v-decorator="['phonenumber', addFormRule.phonenumber]"
              placeholder="请填写手机号码"
            />
          </a-form-item>
          ...
          <a-form-item label="登录账号：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            <a-input
              :disabled="showDetail"
              v-decorator="['loginName', addFormRule.loginName]"
              placeholder="请填写登录账号"
            />
          </a-form-item>
          ...
          <a-form-item label="账号状态：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            <a-radio-group
              :disabled="showDetail"
              name="statusGroup"
              v-decorator="['status', addFormRule.status]"
            >
