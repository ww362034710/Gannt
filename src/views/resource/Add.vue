    ...
    <div class='main-component addDemad-main'>
        ...
        <div class="demand-content">
            ...
            <div class="" v-show="stepsCur===0">
                <select-resource-model @loadResourceModel="loadResourceModel"></select-resource-model>
            </div>
            <!-- 基本信息 -->
            <div class="steps-one" v-show="stepsCur===1">
                <add-basic-info ref="addResourceBasicInfo" :resource-model="resourceData"></add-basic-info>
            </div>
            <!-- 功能信息 -->
...
<script>
import SelectResourceModel from "../../components/resourceModel/SelectResourceModel";
import AddBasicInfo from "../../components/resource/AddBasicInfo";
import FeatureInfo from "../../components/stuff/FeatureInfo";
    export default {
        name: "resourceAdd",
        components: { AddBasicInfo, FeatureInfo,SelectResourceModel},
        data() {
            return {
                loading: false,
                stepsCur: 0,
        ...
        methods: {
            ...
            async loadResourceModel(modelId) {
                //TODO 加载模板信息
                /**
                 * {
                 *      物资模型信息 （物资信息不包括 id）
                 ...
                 */
                let that = this;
                this.$http.postBody("/bs/resourceModel/loadModel", {stuffModelId: modelId})
                    .then( rst => {
                        that.stuffFeatureData = rst.data.stuffFeatureData;
                        delete rst.data.stuffFeatureData;
                        that.resourceData = rst.data;
                    });
                this.stepsCur++;
            },
            //需求拆解提交
            addHandler() {
                this.$refs.addResourceBasicInfo.addForm.validateFields((err, values) => {
                    if(!err){
                        ...
                        this.$http
                            .postBody("/bs/resource/add", Object.assign({}, values, {features: featureParams}))
                            .then(() => {
                                this.$message.success("新增成功", 2);
                                this.$parent.$parent.$parent.tabClose({title: "添加资源"})
                            }).catch(() => {
                        });
                    } else {
