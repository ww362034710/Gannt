    ...
    <div class='main-component addDemad-main'>
        ...
        <div class="demand-content">
            ...
            <div class="steps-one" v-show="stepsCur===0">
                <add-basic-info ref="addGoodsBasicInfo" v-if="goodsModel !== null" :goodsModel="goodsModel"></add-basic-info>
            </div>
            <!-- 功能信息 -->
            <div v-show="stepsCur===1" class="lastSteps">
                <feature-info ref="addStuffModelFeatureInfo" v-if="stuffModelFeatureList !== null" :featureData="stuffModelFeatureList" :goodsModelId="goodsModelId"></feature-info>
            </div>
        </div>
...
import FeatureInfo from "../../components/stuff/FeatureInfo";
    export default {
            ...
            addHandler() {
                this.$refs.addGoodsBasicInfo.addForm.validateFields((err, values) => {
                    if(!err){
                        ...
                        this.$http
                            .postBody("/bs/goodsModel/edit", Object.assign({}, values, {features: featureParams}))
                            .then(() => {
        ...
        created() {
            if (this.$route.params.goodsModelId !== undefined){
                this.goodsModelId = this.$route.params.goodsModelId;
                let that = this;
                this.$http.postForm("/bs/goodsModel/show", Object.assign({}, {id: this.goodsModelId}))
                    .then(rst => {
                        that.goodsModel = rst.data;
                    }).catch(() => {})
                this.$http.postBody("/bs/stuffModelFeature/list", Object.assign({}, {stuffModelId: this.goodsModelId}))
                    .then(rst => {
