...
export default {
    ...
    methods: {
        ...
        queryRoot(data){
             ...
             //先把基本信息也就是根节点信息查出来显示
            //分解出的信息先在后头查着 万一再查的慢
            this.$http.postBody("/bs/deprecated/goal/goalList", params )
                .then(res => {

                    console.log(res);
                    debugger
                    //先去赋值
                    this.$refs.GoalAdd.setFields(res.rows[0]);
                    //然后再查下一步中的信息
                    this.getTree(res.rows[0]);
                })
                .catch(() => {
                    this.loading = false;
                });
        ...
        getTree(data) {
            ...
            let params = {resolveSchemeId:data.resolveSchemeId};
            console.log(params);
            this.$http.postBody("/bs/deprecated/goal/queryTree", params)
                .then(res => {
                    // this.fatherPagination.total = data.total;
                    // this.fatherItemTblData = data.rows;
                    console.log(res);
                    // this.tree = JSON.stringify(data);
                    // this.chartTreeData = data;
                    // for (let i=0;i<data.length; i++){
                    //   this.chartTreeData.name = data[0].title;
                    //   if (data.children.length>0){
                    //     this.chartTreeData.name = data.children[i].title;
                    //   }
                    // }
                })
                .catch(() => {
