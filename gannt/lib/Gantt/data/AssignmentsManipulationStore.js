    ...
    async applyChanges() {
        ...
        if (assignmentManipulationRecords.length || removedAssignmentManipulationRecords.length) {
            ...
            assignmentManipulationRecords.forEach(assignmentManipulationRecord => {
                ...
                if (realAssignment) {
                    realAssignment.units = assignmentManipulationRecord.units;
                }
                // New one
                else {
                    toAdd.push({
            ...
            // TODO: projectRefreshThreshold seems no longer used
            replica.projectRefreshThreshold = 0;

            result = await projectEvent.commitAsync();

            // change stored event.generation to indicate that the event was changed
            // in case user edits the same event assignments again
            me._projectEventGeneration--;
