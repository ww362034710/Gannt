 ...
 *
 * @typings SchedulerPro/feature/PercentBar -> SchedulerPro/feature/SchedulerProPercentBar
 */
export default class PercentBar extends InstancePlugin {
    ...
    onTaskDataGenerated(taskData) {
        const { taskRecord, children } = taskData;

        if (!taskRecord.milestone && !this.disabled) {
            children.unshift({
                className : 'b-gantt-task-percent',
                dataset   : {
                    percent        : Math.round(taskRecord.percentDone),
