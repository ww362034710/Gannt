...
export default class Field extends Badge(Widget) {
    ...
    get inputElement() {
        ...
        if ('inputWidth' in me) {
            config.style = `${me.inputWidth}${typeof me.inputWidth === 'number' ? 'px' : ''}`;
        }
    ...
    set triggers(triggers) {
        ...
        if (me.clearable) {
            (triggers || (triggers = {})).clear = ObjectHelper.assign({
                cls : 'b-icon-remove',
                handler() {
        ...
        for (const triggerRef in triggers) {
            myTriggers[triggerRef] = Widget.create(ObjectHelper.assign({
                type      : 'trigger',
    ...
    updateReadOnly(readOnly) {
        ...
        // Widget has a say too. It adds the class and fires the event
    ...
    syncInputFieldValue(skipHighlight = false) {
        ...
        if (input && !me.inputting) {
            ...
            // TODO : This highligthing logic needs extracting/clarifying
    ...
    updateRequired() {
        ...
        if (!me.isConfiguring && me.required && me.isEmpty) {
