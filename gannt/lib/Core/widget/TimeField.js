...
export default class TimeField extends PickerField {
    ...
    set value(value) {
        ...
        if (!me.hasChanged(oldValue, newValue)) {
            // But we must fix up the display in case it was an unparseable string and the value therefore did not
            // change. This should not trigger any highlight
            if (!me.inputting) {
    ...
    /**
     * The `step` property may be set in Object form specifying two properties, `magnitude`, a Number, and `unit`, a
     * String.
     *
    ...
    /**
     * Get/Set format for time displayed in field (see {@link Core.helper.DateHelper#function-format-static} for
     * formatting options)
     * @property {String}
     */
    set format(value) {
