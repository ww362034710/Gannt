...
export default class Model extends TreeNode(ModelStm()) {
    ...
    setData(toSet, value) {
        ...
        Object.keys(toSet).forEach(fieldName => {
            const
                field      = this.constructor.fieldMap[fieldName],
                dataSource = field?.dataSource ?? fieldName;

            if (dataSource) {
                if (field?.complexMapping) {
                    ObjectHelper.setPath(this.data, dataSource, toSet[fieldName]);
                }
                else {
                    this.data[dataSource] = toSet[fieldName];
                }
            }
        });
    }
    ...
    */
    ...
    copy(newId = null, params) {
        ...
        if (newId && typeof newId === 'object') {
            ...
            if ('deep' in newId) {
                ...
                VersionHelper.deprecate('Core', '5.0.0', '`deep` attribute deprecated, in favor of a separate second parameter');
    ...
    unJoinStore(store, isReplacing = false) {
        ...
        if (stores.includes(store)) {
            store.unregister(me);
            me.children && me.children.forEach(child => child.unJoinStore(store, isReplacing));
