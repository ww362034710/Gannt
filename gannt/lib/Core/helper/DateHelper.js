...
const
    ...
    formats                    = {
        ...
        // ISO week, 1, 2, ...
        W : date => {
            // Modified version of https://github.com/datetime/current-week-number (MIT licensed)
...
export default class DateHelper extends Localizable() {
    ...
    static makeKey(ms) {
        ...
        const cached = keyCache[typeof ms === 'number' ? ms : ms.getTime()];

        if (cached) {
            return cached;
        }

        return keyCache[ms] = DH.format(typeof ms === 'number' ? new Date(ms) : ms, 'YYYY-MM-DD');
    }
    ...
    static formatDelta(delta, options) {
        ...
        for (unitName of deltaUnits) {
            ...
            if (d || (done && !result.length)) {
                result.push(`${d}${sep}${getUnit.call(DH, unitName, d !== 1)}`);  // TODO 0 is plural in EN but...?
