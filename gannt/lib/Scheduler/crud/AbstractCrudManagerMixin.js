...
export default Target => class AbstractCrudManagerMixin extends Delayable(Target) {
    ...
    hasCrudStore(store) {
        return this.crudStores && this.crudStores.some(s => s === store || s.store === store || s.storeId === store);
    }
            ...
            if (me.trigger('beforeLoad', { pack }) !== false) {
                ...
                me.activeRequests.load = {
                    ...
                    reject(...args) {
                        ...
                        fetchOptions.success = fetchOptions.failure = null;
                        reject(...args);
                    },
                    id   : pack.requestId,
                    desc : me.sendRequest(fetchOptions)
                };
