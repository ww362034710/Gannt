...
export default class Dependencies extends InstancePlugin.mixin(
    ...
    attachToResourceStore(store) {
        ...
        store.on({
            name    : 'resourceStore',
            change  : this.onResourceStoreChange,
    ...
    attachToDependencyStore(store) {
        ...
        store.metaMapId = this.client.id;
    ...
    async onDependencyChange({ action, record, records }) {
        ...
        if (!client.isPainted || me.disabled || client.suspendRendering) {
            return;
        }
    ...
    getDependencyAssignmentsAsFromToArray({ dependency, assignmentStore }) {
        ...
        if (fromEvent && toEvent) {
            const
                // Removed event has no assignments
                fromAssignments = fromEvent.isRemoved ? assignmentStore.removed.values.filter(assignment => assignment.event === fromEvent) : fromEvent.assignments,
                toAssignments = toEvent.isRemoved ? assignmentStore.removed.values.filter(assignment => assignment.event === toEvent) : toEvent.assignments;
    ...
    getBox(dependency, source, assignmentData = null) {
        ...
        // Single, get resource from event
            ...
            // resourceRecord = this.dataApi.getEventResource({ event : eventRecord, resourceStore : this.resoureStore });
            resourceRecord = eventRecord.resource;
        }

        return this.client.getResourceEventBox(eventRecord, resourceRecord, true);
    }
    ...
    getDependencyBounds(dependency, assignmentData) {
        const
            me        = this,
            scheduler = me.client,
        ...
        const
            ...
            startRectangle = me.getBox(dependency, true, assignmentData),
            endRectangle   = me.getBox(dependency, false, assignmentData);
            ...
            if (dependencyBounds) {
                ...
                for (x = boxLeft; x <= boxRight; x++) {
                    ...
                    for (y = boxTop; y <= boxBottom; y++) {
                        ...
                        if (!cacheY.find(i => i.dependency.id === dependency.id && i.metaId === metaId)) {
    ...
    scheduleRefreshDependency(dependency, assignmentData = null) {
        ...
        if (!this.drawnDependencies.find(d => d.dependency === dependency)) {
            return;
        }
    ...
    draw(reLayout = false) {
        ...
        if (!me.disabled && !me.client.suspendRendering && me.client.store.count && scheduler.rowManager.rowCount) {
            const
