...
var _bryntum$scheduler = bryntum.schedulerpro,
    ...
    Widget = _bryntum$scheduler.Widget; // Load localization
...
var Shared = /*#__PURE__*/function (_Localizable) {
  ...
  function Shared() {
    ...
    me.productFullName = productFullName; // Only do theme restoration if we are using a standard theme

    if (themeInfo && themes[themeInfo.name.toLowerCase()]) {
      var theme = BrowserHelper.queryString.theme || BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme;
  ...
  _createClass(Shared, [{
    ...
    value: function insertHeader() {
      ...
      var tools = document.getElementById('tools'),
          fullscreenButton = Widget.create({
        type: 'button',
      ...
      var button = this.infoButton = Widget.create({
        type: 'button',
        ref: 'infoButton',
    ...
    value: function loadDescription() {
      ...
      AjaxHelper.get(url, {
        ...
        button.menuItems = [{
          ...
          editable: false,
          value: StringHelper.capitalize(BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme),
          items: themes,
          store: {
            filters: productName === 'calendar' ? [{
              filterBy: function filterBy(r) {
                return r.value === 'stockholm' || r.value === 'material';
              }
            }] : []
          },
