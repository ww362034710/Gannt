import { LocaleManager, Localizable, AsyncHelper, AjaxHelper, DomHelper, Tooltip, Toast, BrowserHelper, StringHelper, DataGenerator, Fullscreen, Events, GlobalEvents, VersionHelper, EventHelper, Popup, ResizeHelper, Rectangle, Widget } from '../../build/schedulerpro.module.js';
// Load localization
...
class Shared extends Localizable(Events()) {
    ...
    insertHeader() {
        ...
        const
            button      = this.infoButton = Widget.create({
                type       : 'button',
                ref        : 'infoButton',
                ...
                ? [themeCombo = {
                    ...
                    store       : {
                        filters : productName === 'calendar' ? [{
                            filterBy : r => r.value === 'stockholm' || r.value === 'material'
                        }] : []
                    },
                    onAction : ({ value }) => {
                        me.applyTheme(value);
